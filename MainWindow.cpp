...
void MainWindow::onBtnSystemClicked()
{
    ...
    if (!systemManageWindow) {
        systemManageWindow = new SystemManageWindow(this);
        ui->stackedWidget->addWidget(systemManageWindow);

        // 连接状态消息信号到主窗口状态栏
        connect(systemManageWindow, &SystemManageWindow::statusMessageRequested,
                this, [this](const QString &message, int timeout) {
                    ui->statusbar->showMessage(message, timeout);
                });
    }
    ui->stackedWidget->setCurrentWidget(systemManageWindow);
}
